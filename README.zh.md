...
> Fork 自 [virattt/dexter](https://github.com/virattt/dexter)。本版本在原版基础上增加了对 **A 股（中国股票市场）的支持**，数据来源为 [Tushare](https://tushare.pro)。
...
- **A 股支持**：通过 Tushare 查询 A 股数据（历史行情、基本面、财务报表）
...
- Tushare API Token（[注册地址](https://tushare.pro/register)）— 用于 A 股数据
...
本 Fork 集成了 [Tushare](https://tushare.pro)，支持对上交所和深交所 A 股上市公司进行研究分析。
...
**配置步骤：**
1. 在 [tushare.pro](https://tushare.pro/register) 注册并获取 API Token
2. 添加到 `.env` 文件：
   ```
   TUSHARE_API_KEY=your-tushare-api-key
   ```
